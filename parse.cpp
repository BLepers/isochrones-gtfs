...
int string_to_time(string time) {
	return (char_to_int(time[0]) * 10 + char_to_int(time[1])) * 60 + (char_to_int(time[3]) * 10 + char_to_int(time[4]));
}
...
void add_edge(int v, int dst, int travel_time, int departure_time, string trip_id) {
	// cout << "Adding edge from " << stop_ids[v]->stop_name << " to " << stop_ids[dst]->stop_name << " at " << h(departure_time) << " travel time " << h(travel_time) << "\n";
	assert(v < nb_stops);
	int index = vertices[v].nb_edges;
	for (int i = 0; i < index; i++) {
		if (vertices[v].edges[i].dst == dst && vertices[v].edges[i].departure_time == departure_time) {
...
void create_trajectories(char *dir) {
	...
	while (in.read_row(trip_id, arrival_time, departure_time, stop_id, stop_sequence)) {
		...
		/* New destination (we arrive at arrival_time and depart from that station at departure_time) */
		current = stops[stop_id];
		if (!current) {
		...
		if (0) {
			print = 1;
			cout << "Line " << line_number << " " << parent->stop_name << " to " << current->stop_name << " at " << h(string_to_time(arrival_time)) << " == " << trip_id << "," << arrival_time << "," << departure_time << "," << stop_id << "," << stop_sequence << "\n";
		}
		...
		if (trip_id != last_trip_id) {
			...
			if (print)
				cout << "Line " << line_number << " New Seq\n";
		} else if (stop_sequence <= last_stop_sequence) {
			// Same trip, we are expecting the next stop (sequence + 1 usually but can be + anything positive)
			// But some french datasets are completely nonsensical, just ignore the buggy trip fully
			cerr << "Line " << line_number << " Inconsistent stop_sequence\n";
		...
		if (print)
			cout << "TripID " << trip_id << " ServiceID " << t->service_id << "\n";
		if (!info)
			goto skip;
		if (print)
			cout << "Skip1\n";
		...
		if (is_frequently_cancelled(info))	// Do not used cancelled trains in sssp
			goto skip;
		if (print)
			cout << "Not skipped and first = " << first_stop_sequence << " parent " << parent->stop_name << "\n";
...
void add_connection(Stop *dst, Source *f, int iteration, int print) {
	...
	if (f->travel_time < dst->best_time + 60 && f->travel_time < dst->best_time * 2) {
		if (print && f->parent->stop_name == "Bern") {
		...
		if (already_there && dst->best_source != f)
			delete f;
		if (print && !already_there && f->parent->stop_name == "Bern") {
