...
#define NB_QUARTER_HOURS (96)
int time_to_quarter_hour(int time) {
	return time / 15;
}
...
void output_stop(Source *f) {
	...
	std::map<int, int> fastest;	 // departure time -> fastest travel
	for (auto s : dst->parents) {
		int departure_time = s->arrival_time - s->travel_time;
		auto old = fastest.find(departure_time);
